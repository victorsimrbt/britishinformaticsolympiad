...
def skirmish():
    global red_pos,red_face,blue_face,blue_pos
    print("R",red_pos,red_face)
    print("B",blue_pos,blue_face)
    
    if (tuple(hexagons[red_pos][red_face-1]) in edges) and edges[tuple(hexagons[red_pos][red_face-1])] == "B":
        print("SWITCH OWNERSHIP")
        edges[tuple(hexagons[red_pos][red_face-1])] = "R"
        return True
    
    edges[tuple(hexagons[red_pos][red_face-1])] = "R"
    
    if (tuple(hexagons[blue_pos][blue_face-1]) in edges) and edges[tuple(hexagons[blue_pos][blue_face-1])] == "R":
        print("SWITCH OWNERSHIP")
...
for r in range(1,26):
    for b in range(1,26):
        ...
        blue_face = 6
        edges = {}
        print("testing with {} {}".format(r,b))
        counter = 0
        end = False
        while not(end) and counter < 1000:
            counter += 1
            end = skirmish()
            print()
        if counter > maximum and end:
            maximum = counter
        if counter < minimum and end:
            minimum = counter
            # print("MIN",r,b,minimum)
        print(counter)
        # time.sleep(1)
print(maximum)
print(minimum)
